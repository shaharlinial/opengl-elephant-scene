...
SpotLight::SpotLight(GLenum ln) :light_name(ln),
position{ 7.4f, 1.9f, 7.4f , 1.0f },
target{ 5.0f, 0.32f, 4.67f },
color{ 1.0f, 0.980f, 0.672f },
cutoff(17.0f),
exponent(20.0f) {}

void SpotLight::addlight() {
	if (!glIsEnabled(light_name))
		return;
	glLightfv(light_name, GL_DIFFUSE, color);
	glLightfv(light_name, GL_SPECULAR, color);
	glLightfv(light_name, GL_POSITION, position);
	GLfloat direction[3] = { target[0] - position[0],
		target[1] - position[1],
		target[2] - position[2] };
	glLightfv(light_name, GL_SPOT_DIRECTION, direction);
	glLightf(light_name, GL_SPOT_CUTOFF, cutoff);
